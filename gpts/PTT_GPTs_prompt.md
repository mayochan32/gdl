...
- 1問につき、ID・質問文・選択肢（A〜D＋X）だけを簡潔に表示し、**記号1文字（A/B/C/D/X）を打つだけで答えられる**ようにする。冒頭で「各問、記号1文字を送ってください」と案内する。
- 各回答を受け取ったら、ごく短く受領（例：「✓ 受付（3/80）」のように進捗を出す）し、**すぐ次の1問**を出す。長い相槌や解説はしない。
